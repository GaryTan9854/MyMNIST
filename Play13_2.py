...
class Hand13(Hand):
    ...
    def play_strategy1(self, df):
        ...
        if not atk:
            ...
            if indx0!=indx:
                ...
                s3 = df['s3'][indx]
                atk = self.eval_CanAttack(s1,s2,s3)
                print(df.loc[indx], s1,s2,s3, indx, self.defense_score, atk)
    ...
    def arrange13(self): #return list of [3,5,5]
        ...
        df.to_csv (r'hand_st1.csv', index = 'sc1', header=True)
...
class Game(object):
    ...
    def compete(self, h1, h2):
        ...
        #碾壓規則: 兩敦贏一敦平手
        if sorted(res[0:3])==[0,1,1]: # 難道不會出現別的情形？
            res=[1,1,1,3,1]
        tot=sum(res)  #要先加原始分數
...
sample_study = ['03D','04C','05H','07S','08C','08S','09H','11C','12C','13C','13H','13D','13S']
 ...
 # => fixed for raising FH score
 # [ ♠2 ♣3 ♢3 ♢4 ♣6 ♣7 ♢8 ♠8 ♡8 ♡9 ♣10 ♠J ♡A ] 
 # ==> 順的 sc2 還是太大 已再調高葫蘆的值
 # [ ♠2 ♢3 ♠3 ♠6 ♠7 ♣10 ♢10 ♡10 ♢Q ♠Q ♢K ♡K ♣K ] 
sample_st2= ['02S','03D','03S','06S','07S','10C','10D','10H','12D','12S','13D','13H','13C']
 # 把 QQ 上推的可能 => 把attack 首敦改成 5.5x 居然出現 10衝三 interesting sample
#  [ ♣3 ♢4 ♣5 ♡5 ♡6 ♠7 ♡8 ♠10 ♢J ♣Q ♡Q ♠Q ♠K ] 
# J亂 k亂 Q葫 vs k亂 q三條 順 => s2 232.6 vs 239.48 這局這樣排沒被打槍，不然會遇 A葫
