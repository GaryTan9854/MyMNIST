...
# -*- coding: utf-8 -*-
"""
Created on Sun Sep 12 16:32:51 2021
...
class AmoebaWorld(World):
    ...
    def make_canvas(self, low=-10, high=10):
        ...
        self.ca_height = 400
        self.canvas = self.ca(
            width=self.ca_width, height=self.ca_height, bg="white", scale=[20, 20]
        )
...
class Amoeba(Animal):
    ...
    def draw(self):
        ...
        self.world.canvas.polygon(coords, fill=slime, outline=slime, tags="slime")

        # draw the outer perimeter
        self.world.canvas.polygon(
            coords, fill=self.color1, outline=self.color2, tags=self.tag
        )

        # draw the perimeter of the nucleus
        coords = self.poly_coords(self.x, self.y, thetas, self.size / 2)
        self.world.canvas.polygon(
            coords, fill=self.color2, outline=self.color1, tags=self.tag
        )
